...
const RecipeDetailScreen = () => {
  ...
  useEffect(() => {
    const checkIfSaved = async () => {
      if (!userId) return; // Don't make API call if no user
      try {
        ...
        if (!response.ok) {
          // If server is not available, just skip the check
          return;
        }
        ...
        // Silently fail - don't show error to user for this check
    ...
    // Only check if saved if we have a valid user ID
    if (userId) {
      checkIfSaved();
    }
    loadRecipeDetail();
